...
extension Observable {
    func merge(other: Observable<Element>) -> Observable<Element> {
...
final class RangeSlider: UIView {
    ...
    @IBInspectable var inactiveBarColor: UIColor = .systemRed
    @IBInspectable var activeBarColor: UIColor = .systemGreen
    ...
    private func configureUI() {
        ...
        if debug {
            ...
            quarterView.backgroundColor = .systemOrange
            middleView.backgroundColor = .systemIndigo
            lastQuarterView.backgroundColor = .systemYellow
...
var slider: RangeSlider? = RangeSlider(frame: CGRect(x: 0, y: 0, width: 300, height: 25))
PlaygroundPage.current.liveView = slider // prevents deinit
Observable.just((min: 0.25, max: 0.75))
    .bind(to: slider!.rx.initialSelection)
slider?.observeResult
    .subscribe(onNext: {
        print($0)
    })
//slider = nil // test deinit
